...
class MyThingy {
    ...
    startSimulation() {
        ...
        const simThingy = {
            ...
            _onButtonChange: (state) => console.log(state ? 'Plug switched ON' : 'Plug switched OFF'),
    ...
    async switchHs100(state) {
        const self = this;
        return new Promise(function (resolve, reject) {
            if (self.uuid === 'simulation') {
                self.state = ! self.state;
                self.led_set(null, () => console.log(self.state ? 'Plug switched ON' : 'Plug switched OFF'));
    ...
    _onButtonChange(state) {
        if (state === 'Pressed') {
            if (this.startSensorsStatus === true) {
                ...
                this.switchHs100(true).then(()=>console.log('ON'));
            } else {
                this.startSensorsStatus = true;
                this.cb(false);
                this._enableSensors();
                this.switchLight(1).then(() => console.log('Enabled sensors data'));
                this.switchHs100(false).then(()=>console.log('OFF'));
            }
        }
    }
